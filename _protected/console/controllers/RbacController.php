...
class RbacController extends Controller
{
    ...
    public function actionInit()
    {
        ...
        $userModel->username = 'administrator';
        $userModel->email = 'admin@admin.com';
        $userModel->password = 'administrator';
        $userModel->setPassword($userModel->password);
        $userModel->generateAuthKey();
        $userModel->status = User::STATUS_ACTIVE;
        // $userModel->kd_user = 1;
        $userModel->nama = 'Administrator Aplikasi';
        $userModel->nip = '000000000000000000';
        $userModel->item_name = 'admin';

        $userModel->save(false);

        $authUser = Yii::$app->authManager;
        $role = $authUser->getRole('admin');
        $info = $authUser->assign($role, $userModel->getId());
